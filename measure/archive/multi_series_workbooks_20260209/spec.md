...
- **Git-clean root**: After migration, the `WORKBOOKS_ROOT` root should contain only `primary/`, `secondary/`, `dashboard/`, `measure/`, and config files.

## Acceptance Criteria

1. Running `ls` on `WORKBOOKS_ROOT` shows `primary/`, `secondary/`, `dashboard/`, `measure/`, and config files — no loose project dirs or test artifacts.

...
| 2026-04-14 | lesson_editor_refactor_20260413 | Lesson editor page.tsx is ~900 lines; consider extracting sub-editors per section for maintainability | Medium | In Progress | Phase 1-3 complete: BasicInfoEditor, ArticleEditor, VocabularyEditor, PedagogicalConnectorsEditor extracted; page.tsx reduced to ~650 lines |
| 2026-04-21 | primary_pipeline_bugs | AI augmentor PedagogicalContentSchema enforces max(3) on writing_sentence_frames but primary lessons have 4; Gemini may violate constraint causing Zod validation failure. Also: content_qa uses z.union which converts to anyOf — may not be supported by Gemini responseJsonSchema. Fix: relax min/max constraints to match WorkbookLessonSchema. Legacy primary content may also still carry `article_image_url` arrays instead of schema-compliant `article_images`, which load but do not survive schema-backed editing cleanly. | High | Open | Fails on primary (A0/A1) lessons; secondary appears unaffected. Confirm with server logs before fixing. |
| 2026-04-26 | primary_compile_layout_papercuts | Primary Step 11 Guided Writing is still content-length sensitive in Paged.js; longer prompts, prompt images, or QR feedback copy can push the bordered writing exercise across pages even after compact spacing changes. | Medium | Open | Consider a content-aware compact mode or separating digital follow-up from the printable writing box if overflow persists. |
| 2026-04-26 | thai_vocab_backfill_origins_a0 | Primary student lesson templates originally rendered English-only vocabulary and matching text even though glossary/flashcards/teacher guide already supported `thai_definition`; this encourages content-only fixes that never reach the core worksheet pages. | Medium | Resolved | Added `thai_definition` rendering to primary/secondary lesson templates and extended `vocab_match` schema support so bilingual content survives schema-backed edits. |
| 2026-04-26 | primary_compile_layout_papercuts | Primary Step 3 article layout is also sensitive in Paged.js: QR intro blocks can orphan the section header, and float-based inline images may clip or split badly when the article fragments across pages. | Medium | Open | Current mitigation is to start Step 3 on a fresh printed page and avoid `overflow:auto` on the article container. Watch for regressions when adjusting QR or article image layout. |

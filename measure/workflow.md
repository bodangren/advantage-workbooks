...
8. **Update Project Memory:**
   - If new tech debt was introduced or existing debt resolved, update `measure/tech-debt.md`.
   - If important gotchas, design decisions, or patterns emerged, update `measure/lessons-learned.md`.
...
3. **Archive the Track Folder:** Move the entire track directory from `measure/tracks/<track_id>` to `measure/archive/<track_id>`. Create the `archive` directory if it does not exist.
4. **Update Registry:** Remove the track's entry completely from `measure/tracks.md`.
5. **Commit Archive:** Commit the archiving action with a message like `chore(measure): Archive completed track '<track_description>'`.

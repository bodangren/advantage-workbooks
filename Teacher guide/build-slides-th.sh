...
# Build script for Reading Advantage Training Slides (Thai)
# Generates PDF presentation from markdown

echo "Building Reading Advantage Training Slides (Thai)..."
...
if command -v marp &> /dev/null; then
    ...
    marp training-slides-th.md \
        --pdf \
        --allow-local-files \
        --theme slides-theme.css \
        --output training-slides-th.pdf

    if [ $? -eq 0 ]; then
        echo "✓ PDF generated successfully: training-slides-th.pdf"
    else
    ...
    pandoc training-slides-th.md \
        -o training-slides-th.html \
        --standalone \
        --css=slides-theme.css \
        --metadata title="การอบรมครูผู้สอน Reading Advantage" \
        --self-contained \
    ...
    echo "✓ HTML generated: training-slides-th.html"
    ...
    if command -v wkhtmltopdf &> /dev/null; then
        ...
        wkhtmltopdf \
            ...
            training-slides-th.html \
            training-slides-th.pdf

        if [ $? -eq 0 ]; then
            echo "✓ PDF generated: training-slides-th.pdf"
        else
...
echo "Output files:"
if command -v marp &> /dev/null; then
    echo "  - training-slides-th.pdf (PDF presentation)"
else
    echo "  - training-slides-th.html (HTML presentation)"
    if command -v wkhtmltopdf &> /dev/null; then
        echo "  - training-slides-th.pdf (PDF presentation)"
    fi
fi
